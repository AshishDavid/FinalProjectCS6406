...
Download the dataset from [here](https://www.mvtec.com/company/research/datasets/mvtec-loco/).
...
Please specify dataset path (line1) and log folder (line10) in `run.sh` before running.

`run.sh` gives the configuration to train models on MVTec LOCO dataset.
```
bash run.sh
```
