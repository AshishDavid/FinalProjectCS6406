...
class SimpleNet(torch.nn.Module):
    ...
    def _evaluate(self, test_data, scores, segmentations, features, labels_gt, masks_gt):
        ...
        # LOGICAL VS STRUCTURAL AUROC FOR MVTEC LOCO
        logical_indices = [i for i, x in enumerate(test_data.dataset.data_to_iterate) if x[1] == "logical_anomalies" or x[1] == "good"]
        structural_indices = [i for i, x in enumerate(test_data.dataset.data_to_iterate) if x[1] == "structural_anomalies" or x[1] == "good"]
        
        logical_auroc = -1
        structural_auroc = -1
        
        if len([i for i in logical_indices if test_data.dataset.data_to_iterate[i][1] != "good"]) > 0:
            logical_scores = scores[logical_indices]
            logical_labels = np.array(labels_gt)[logical_indices]
            logical_auroc = metrics.compute_imagewise_retrieval_metrics(logical_scores, logical_labels)["auroc"]
            
        if len([i for i in structural_indices if test_data.dataset.data_to_iterate[i][1] != "good"]) > 0:
            structural_scores = scores[structural_indices]
            structural_labels = np.array(labels_gt)[structural_indices]
            structural_auroc = metrics.compute_imagewise_retrieval_metrics(structural_scores, structural_labels)["auroc"]
            
        print(f"Logical AUROC: {logical_auroc}, Structural AUROC: {structural_auroc}")
